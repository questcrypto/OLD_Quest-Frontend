...
const ApprovedProperty = (props: any) => {
  const classes = useStyles()
  const [loading, setLoading] = useState(false)
  const [selectedId, setSelectedId] = useState('')
  const { data, approvedLoading, userInfo, setActiveTab, updateApprove } = props
          ...
          <Table aria-label="simple table">
            <TableHead>
              <TableRow>
                ...
                {!!userInfo && userInfo.role === 1 && <TableCell>ACTION</TableCell>}
              </TableRow>
            </TableHead>
            {!!data && data.length > 0 && (
                    ...
                    {!!userInfo && userInfo.role === 1 && (
                      <TableCell>
                        <Button
                          ...
                          variant="contained"
                          classes={{
                            root: classes.addPropertyBtn,
                          }}
                          onClick={() => handleApproveByAdmin(row.id)}
                        >
...
export default ApprovedProperty
